...
class Dao {
	 ...
	 * @throws	Exception	Si l'objet de critère n'est pas bon.
	 */
	public function __construct(FineDatabase $db, /* FineCache */ $cache=null, $tableName=null, $idField='id', $dbName=null, $fields=null, $criteriaObject=null) {
		...
		if (!empty($criteriaObject)) {
			if (!is_subclass_of($criteriaObject, '\AppBundle\FineDao\DaoCriteria'))
	 ...
	 * @return 	DaoCriteria	L'objet de critère.
	 */
	 ...
	 * @param	DaoCriteria	$criteria	(optionnel) Critères de recherche.
	 * @return	int	Le nombre.
	 */
	public function count(DaoCriteria $criteria=null) {
    ...
    /**
     * Retourne l'id maximum
     * @return	int IdMac
     */
    public function getIdMax() {
        $cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ':idMax';
        $sql = 'SELECT MAX(' . $this->_idField . ') AS idMax
			FROM ' . (empty($this->_dbName) ? '' : ($this->_dbName . '.')) . "`" . $this->_tableName . "`";
        // on cherche la donnée en cache
        if (($res = $this->_getCache($cacheVarName)) !== null)
            return ($res);
        // exécution de la requête
        $data = $this->_db->queryOne($sql);
        // verifications
        if (empty($data)){
            unset($cacheVarName, $sql, $data);
            return (null);
        }
        // decryptage et indexation
        $res = $data['idMax'];
        // écriture de la donnée en cache
    ...
    /**
     * Retourne tous les identifiants de la table
	 *
	 * @param	$where	(optionnel)	chaine where. Ex (where company_id = 2)
     * @return	array	Tableau d'identifiants.
     */
    public function getAllIds($where=null) {
        $sql = 'SELECT ' . $this->_idField . ' AS id
			FROM ' . (empty($this->_dbName) ? '' : ($this->_dbName . '.')) . "`" . $this->_tableName . "` " .
			$where .
            ' ORDER BY ' . $this->_idField . ' ASC ';
        $cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ':request:' . md5($sql);
        // on cherche la donnée en cache
        if (($res = $this->_getCache($cacheVarName)) !== null) {
     ...
     *
     * Attention, cette fonction peut etre gourmande en ressource memoire et reseau
     */
    public function getAll() {
        // prise du cache
        $cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ':all';
    ...
    /**
     * Envoie une requete et retourne son contenu
	 * DOIT etre une requete sur une table
	 *
	 * @param	string	$sql
     * @return	array	Tableau d'entites.
     *
    ...
    /**
	 * Recupere a partir d'ids
	 *
     * @param array $myIds
	 *
	 * @return array|null Liste des entites trouvees.
     */
	public function getFromIds($myIds) {
		// verifications
		if (empty($myIds) || !is_array($myIds))
			return null;
        ...
        // requete
		{
            $sql = 'SELECT ' . $this->_getFieldsString() . ' FROM ' .
                (empty($this->_dbName) ? '' : ($this->_dbName . '.')) . "`" . $this->_tableName . "`" .
				' WHERE `' . $this->_idField . "` IN ('" . implode("', '", $ids2)  . "') ";
		}

		// prise du cache
		{
			// cle de cache
            $cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ":get:" . md5($sql);
            if (($data = $this->_getCache($cacheVarName)) !== null) {
            	unset($ids2, $sql, $cacheVarName);
                return ($data);
            }
		}
		...
		// prise en DB
        {
            $data = $this->_db->queryAll($sql);
            if (empty($data)) {
                unset($ids2, $sql, $cacheVarName, $data);
                return null;
            }
        }
	 ...
	 * @throws	Exception	Si les données reçues sont mal formées ou Si l'insertion s'est mal déroulée.
	 */
	public function create($data, $safeData=null) {
		...
		foreach ($data as $key => $value) {
			if (!is_string($value) && !is_numeric($value) && !is_bool($value))
				throw new Exception("Bad field value for key '$key'.");
	 ...
	 * @param	DaoCriteria	$criteria	(optionnel) Critères de recherche. Null par défaut, pour prendre tous les enregistrements.
	 ...
	 */
	public function search(DaoCriteria $criteria=null, $sort=null, $limitOffset=null, $nbrLimit=null) {
		$cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ':count';
		$sql = 'SELECT ' . $this->_getFieldsString() . ' FROM ' .
			(empty($this->_dbName) ? '' : ($this->_dbName . '.')) . "`" . $this->_tableName . "`";
		if (isset($criteria)) {
	 ...
	 * @throws	Exception	Si les critères ou les champs/valeurs sont mal formés.
	 */
	public function update($criteria=null, $fields) {
		...
		else if ($criteria instanceof DaoCriteria)
			$sql .= $criteria->generate();
		else
			throw new Exception("Bad criteria type.");
		$this->_db->exec($sql);
	}
	/**
	 ...
	 * @return	Dao	L'instance de l'objet courant.
	 */
